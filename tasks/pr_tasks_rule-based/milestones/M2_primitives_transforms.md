---
milestone: M2
title: "Primitives completos + transforms base"
status: Draft
...
# M2 — Primitives + transforms base

> Cobrir 80% dos schemas com geradores primitivos e transforms composáveis.
...
Entregar primitives e transforms reaproveitáveis, guiadas por plan, com performance e determinismo.


## Escopo

**Dentro**
- Implementar ≥10 primitives.
- Implementar ≥5 transforms.
- Suporte a `transforms` no plan por coluna.
- Respeitar limites vindos do schema e sinalizar violações.
...
## Catálogo de primitives (IDs e parâmetros)

- `primitive.bool` {p_true?}
- `primitive.int.range` {min, max}
- `primitive.int.sequence_hint` {start, step}
- `primitive.float.range` {min, max}
- `primitive.decimal.numeric` {min, max, scale?}
- `primitive.text.pattern` {len?, charset?, pattern?}
- `primitive.text.lorem` {min_words, max_words}
- `primitive.uuid.v4` {}
- `primitive.date.range` {min, max}
- `primitive.time.range` {min, max}
- `primitive.timestamp.range` {min, max}
- `primitive.json.stub` {template?} (opcional)


## Transforms (IDs e semântica)

- `transform.null_rate` {p}
- `transform.truncate` {max_len? | from_schema=true}
- `transform.format` {...}
- `transform.prefix_suffix` {prefix?, suffix?}
- `transform.casing` {mode: lower|upper|title}
- `transform.weighted_choice` {choices:[{value, weight}]}
...
Ordem recomendada:
1) Generator base
2) Transforms (na ordem do plan)
3) Serialize CSV

Determinismo:
- ordem de transforms preservada
- nada de iteração em HashMap sem ordenar


## Plano de implementação (passo a passo)

1) Criar `generators/primitives/*` e `generators/transforms/*` com módulos pequenos.
2) Atualizar parsing do plan (generator.id + transforms).
3) Integrar aplicação de transforms na engine.
4) Testes unitários: bounds/format/determinismo.
5) Atualizar examples se necessário.
...
- [ ] ≥10 primitives implementadas e registradas.
- [ ] ≥5 transforms implementadas e aplicadas.
- [ ] `resolved_plan.json` reflete defaults/transforms.
- [ ] E2E + determinismo OK.
...
> **Modo de execução:** implemente **somente** esta milestone (M2).  
...
**Tarefa**
- Implementar primitives + transforms descritos.
- Integrar ao plan.
- Adicionar testes.
